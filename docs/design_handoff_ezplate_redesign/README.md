...
- localStorage holds view preferences and derived caches ONLY (nothing new needed — the density toggle was cut).
...
1. **Redesign - Dashboard v2 Desktop.dc.html** — Verdict headline (44px mono, semantic color) + menu selector as ONE simple dropdown right of the verdict ("All menus 41.2 ▾" button opening a ranked list: All menus first, then every menu worst-first with its food-cost % in semantic colour; current pick tinted --accent-weak; uncosted menus excluded) — no chips + full-width trend chart (over-target wash, dashed target line, accent markers for user changes) + AI briefing panel ("What needs attention": sparkle icon 16px, small-caps heading, sentence rows with one quiet link each, credit line "Phrased by Gemini — every figure computed by EzPlate") + What moved + Dig in as two-column row lists.
2. **Redesign - Menu.dc.html** — Real table on desktop: sections as small-caps eyebrows inside one surface; columns Plate / Cost (with min–max range under) / Suggested / Menu price / Margin. The Margin cell states margin vs target ONLY — "27.2% ✓" green, "42.2% · over" amber, "45.5% · over" red, "cost it →" muted for uncosted. Never a dollar delta or suggested price increase: the app judges cost, it does not push price hikes. Filter toolbar wraps below ~1130px.
...
10. **Design Package.dc.html** — The system spec: tokens, type scale, spacing/shape, buttons, fields, chips, rows, margin verdict block, AI panel rules, navigation, motion/state table, empty states, skeletons, keyboard/focus/contrast rules, implementation notes. **Read this first; it is the rulebook.**
11. **Redesign - Error, Tablet & First-run.dc.html** — Addendum (Package §16–18). §16 Error states: restyles the SHIPPED mechanisms only (`toast()`, `#syncBanner` states, boot gate, invoice file-failed) — keep prod's error wording verbatim, failure toasts get a --bad border + icon and hold 5s (success stays 2.2s), every error offers exactly one action, no new error plumbing. §17 Breakpoints: <768 phone / 768–1023 tablet (bottom nav kept, 24px gutters, content max 720px, targets ≥44px, hover only ≥1024 under `@media (hover:hover)`) / ≥1024 desktop shell; the only per-screen wraps are 1120px (builder panel) and 1130px (menu toolbar) — add no others. §18 First run: empty states are the onboarding — Dashboard 3-step path card replaces the verdict hero while nothing is costed (state derived from data, no stored flag, gone once a verdict exists); Products first-run leads with invoice import; Builder/Menu keep their shipped empty-state wording, restyled.
...
## State Management
No new stores. New view state: dashboard scope (existing `dashScope`), menus-dropdown open, invoice tick set (existing `invConfirmState` rules unchanged).
...
8. **Density toggle: cut.** Do not implement Comfortable/Compact or any density localStorage key — one row height everywhere (Package §3).
9. **Gap-closure batches (after the ten screen batches):** G1 error-state restyle (toast/sync pill/boot gate/invoice file-failed — presentation only, wording untouched, parser untouched) · G2 tablet pass (768–1023 layer per §17; test iPad portrait AND landscape) · G3 first-run (Dashboard path card + Products first-run empty state; no new storage). Each is its own PR like any screen batch.
