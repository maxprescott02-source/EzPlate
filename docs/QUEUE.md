...
## done  `v141-sync-corner.spec.js` has failed IN SETUP - the cause is a Chromium SEGFAULT, and it is not confined to that file
**ANSWERED 10 Aug 2026, docs + one CI comment, no deploy version. The CAUSE is named and certain; the residual "why that file more often" is recorded as UNEXPLAINED with the arithmetic and the refuted candidate, rather than declared chance. See the done section - and read it before citing this thread, because the item's own framing was wrong in three ways.**

## next  A browser SEGFAULT reads as a mystery flake, and the diagnosis is one grep nobody runs
Problem: found 10 Aug 2026 by the investigation above. The CI Playwright job now retries a crashed browser and goes green with a `flaky` annotation - correct behaviour - but the annotation says only that a spec passed on a retry. **It does not say the browser segfaulted**, so the reader's first hypothesis is a regression in their own diff, which is the one thing it cannot be.
Diagnosing it once cost an artifact download, an unzip, a trace parse and a hex subtraction. **The evidence needed is a single string** - `Received signal 11` appears in the Playwright error text, which is already inside `playwright-results.json` and already inside the job log.
Requirements: extend the existing `Did any spec pass only on a retry?` step (or add one beside it) so a run whose failure text contains `Received signal` says so in the warning - something like "the browser process crashed (SIGSEGV): this is infra, not your diff". Read it out of the json rather than the log, and it must **fail open** exactly as the existing detector does: node parsing inside a try/catch, printing nothing on a malformed report, never turning a green run red.
Note the count is the honest signal only for `stats.flaky`; here the string is what matters, and `HANDOVER-162` records that the obvious grep for `"status":"flaky"` matches zero times because the reporter pretty-prints. **Check any literal against a real report before shipping it**, which is the whole lesson of that batch.
Out of scope: the retry count, and the upload gate, both correct as they are.

## next  Test the tightest-cycle hypothesis, then bump Playwright / Chromium
Problem: the segfault diagnosed 10 Aug 2026 has a **live, untested hypothesis** that the done entry above sets out - four of six occurrences are the context creation immediately after `v141-sync-corner.spec.js:105`, the suite's shortest test at ~600-1000ms, which does a full page load and service-worker registration and is then torn down.
**Do the cheap experiment BEFORE the bump, because the bump destroys the evidence** - a new Chromium may hide the crash without anyone learning whether the tight cycle caused it, and this repo would then be back to "mystery flake" the next time a spec gets short. Two candidate probes, both one line: lengthen `:105` (a `waitForTimeout` before it ends, so teardown is not racing the registration) or stop the harness registering a service worker at all. Neither is a fix to keep - they are experiments, and whichever runs must be reverted or justified.
⚠️ **It cannot be settled by a green run.** The rate is 6 CI occurrences in 44 runs, so a handful of passes proves nothing either way; this is worth doing only alongside recording every future occurrence with its predecessor test, which the done entry now asks for.
Then the bump: the crash is in `chrome-headless-shell-1228` shipped with Playwright **1.61.1** (`package.json` pins `^1.61.1`), a null dereference at a fixed code offset, so it is a browser bug and a different browser build is the only real fix. Retries hide it competently in the meantime, which is why none of this is urgent.
Requirements: bump `@playwright/test`, run the suite, and **watch the geometry specs specifically** - a Chromium version change moves rendering, and this repo's specs measure computed geometry to the pixel. `CLAUDE.md`'s viewport rule exists because two assertions once passed on macOS and failed only on the Linux runner, so a bump is verified in CI, not locally. The browser cache key is already the resolved Playwright version, so a bump correctly misses the cache and refetches.
Note the bump cannot be shown to have WORKED, only to have not recurred: the measured rate is **6 CI occurrences across the 44 `tests` runs of 10 Aug**, so a handful of green runs proves nothing. Record the version and the date, and treat a later occurrence as evidence against the bump rather than as a new mystery.
Out of scope: any other dependency, and the client's two pinned third-party scripts, which are a different rule entirely (`CLAUDE.md` Tier 2 - and a bump there means recomputing the `sha384` in the same commit).
...
- **The `v141-sync-corner` setup failures - a Chromium SEGFAULT, and the file was never the subject** - answered 10 Aug 2026, **docs + one CI comment, no deploy version** (`sw.js` stays `ezplate-v145`), handover `HANDOVER-163-browser-segv.md`.
  **THE CAUSE, measured, not inferred: the browser PROCESS CRASHES.** All **six** CI occurrences carry `Received signal 11 SEGV_MAPERR 0000000001b0`, and **all six crash at the IDENTICAL instruction, binary offset `0x2af9eec`** (module offset subtracted from the absolute address, so it survives ASLR), in `chrome-headless-shell-1228` under Playwright 1.61.1. `ax` is 0 and `cr2` is `0x1b0` - a null dereference at a fixed member offset; `rcx` holds the ASCII `ocalhost`. It is one bug, six times, not a family of crashes. `--disable-dev-shm-usage` is already on the launch line, so the usual /dev/shm explanation is excluded. **Nothing in the spec's own code is involved** - the item's observation that the file manages no contexts was right, and this is why.
  ⚠️ **"NOTHING IS SLOW" - the item's central number means the opposite of what it says.** It read the 30002ms as "the 30 seconds is consumed entirely by context creation". It is 30 seconds spent **waiting on a process that is already dead**: Playwright's `context` fixture sits in the test timeout when the browser has gone. **The item's "two different errors" are ONE event** - `Target page, context or browser has been closed` and `Test timeout … while setting up "context"` differ only in whether Playwright had noticed yet.
  ⚠️ **THE PREMISE "all in this one file" IS FALSE.** A census of **every `tests` run of 10 Aug** - 42 at the time of the sweep, 44 once this batch had run its own, and occurrence 6 is in one of those two - found **seven** occurrences, not three, in **two** spec files (six in CI, one local):
  `31348972108` (branch, 02:08) `v141:90` @1440 · `31349737666` (branch, 02:25) `v141:90` @1280 · **`31359764333` (main, 05:48) `v142-menu.spec.js:67`** · `31368954663` **attempt 1** (branch, 08:11) `v141:174` · `31380462471` (PR #144, 10:44) `v141:125` · `31384057429` (**PR #145, this batch's own run**, 11:34) `v141:125` · plus the local `:229`, same error string, no log kept.
  The `v142` one is the run that **turned `main` red**, four hours before the item was written asserting no other file had ever done this. It was in the record the whole time. "Three of three in the 5.7% file" was a count of the occurrences someone had looked at.
  ⚠️ **`gh` WILL TELL YOU A RE-RUN NEVER HAPPENED - this cost an hour and will cost the next person one too.** Occurrence 4 is `31368954663` **attempt 1**, the incident that bought `--retries=1`. `gh run list --json attempt` and `gh run view --json attempt` **both report `1`** for that run; only the REST field `run_attempt` (`gh api repos/:owner/:repo/actions/runs?created=…`) says `2`, and only `…/attempts/1/jobs` will give you its log. A first pass over the record concluded the incident was unlocatable, which was wrong. **A re-run hides the original attempt from every `gh run` subcommand.**
  ⚠️ **THE CONCENTRATION IS NOT CHANCE, AND THE PATTERN IS SHARPER THAN "this file".** This entry was drafted twice: first as "chance, here is the arithmetic", then rewritten when this batch's OWN PR run crashed in the same place. **Four of the six CI occurrences are the context creation IMMEDIATELY AFTER a `v141-sync-corner.spec.js:105` test** - occurrences 1, 2 (`:105` → `:90`) and 5, 6 (`:105` → `:125`). That is not a file-level coincidence, it is a specific predecessor.
  **What `:105` is: the shortest cycle in the suite.** It runs in **~600-1000ms** and in that time does a full `page.goto('/')`, which registers a service worker against `http://localhost:5173`, and then the context is torn down. The crash register `rcx` holding the ASCII `ocalhost` is consistent with that and is the reason it is worth writing down rather than filing as noise.
  **The control that IS valid, and what it does and does not rule out.** `v137-modal-layer.spec.js` runs **22 tests at a 1215ms mean** against `v141`'s **12 at 2235ms**, and it does boot a fresh page per test (`boot()` at `:69` wraps the `goto`; a naive grep says 2 gotos and is wrong - it counts the helper once). It has never crashed. So **AVERAGE churn is refuted**; the **tightest individual cycle is not** - `v137`'s shortest test is 770ms and `v141:105` runs at ~605ms. That is the live hypothesis and it is untested.
  **So do not treat a further occurrence as noise.** Record the file, the test, AND the test that ran before it - the predecessor is the datum this took six occurrences to see.
  **What follows: `--retries=1` is not a workaround, it is the correct response.** A segfault is unambiguously infra, cannot be a regression in the diff, and a fresh worker gets a fresh browser. The cost is 30 seconds per occurrence. `test.yml`'s comment block, which stated the disproved mechanism in three places, now states this one and names the grep. Two follow-ups queued: surfacing the crash signature automatically, and the Chromium version bump that is the only actual fix.
...
- **The flaky-but-green Playwright trace** - shipped 10 Aug 2026, **CI-only, no deploy version** (`sw.js` stays `ezplate-v145`), handover `HANDOVER-162-flaky-trace.md`.
  ...
  **The rehearsal went flaky by itself**, on the same spec file as the incident that bought `--retries=1`. **That thread is CLOSED - see the segfault entry above.** It was not a defect in the spec and not a defect in this repo: the browser process segfaults, and "the same spec file" turned out to be a coincidence the census disproved.
