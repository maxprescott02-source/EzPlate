...
## next  F1b - Shell reconcile + the modal/sheet primitive
**Split out of F1 on 10 Aug 2026, during F1a.** F1 as written was tokens + dark + theme switch + shell reconcile + a modal/sheet primitive carrying two behavioural defects. One PR changing every colour in the app AND rewriting every modal's interaction is `CLAUDE.md`'s "exceeds what one PR can be reviewed as" stop condition, whose remedy is exactly this: do the first half, queue the rest. Both halves are shell work, so nothing violates protocol §2 - no screen is touched by either, and no screen is left half-converted.
**F1a shipped the tokens and the theme switch. What is left:**
...
- **The mock's compact sidebar theme toggle** (22px, in the brand row beside a ⌘K button). F1a put the control in Settings instead, because Settings is reachable at every screen size and the sidebar is desktop-only (≥1024) - the mobile mock has no theme control at all. Adding the sidebar toggle is a sidebar change, so it belongs here. Note it is a 22px target in a codebase with a 44px floor: desktop pointer-only, so decide consciously rather than shrinking the rule.
- **Modal/sheet primitive:** NEW - centered 12vh modal ≥768, bottom sheet <768, scrim + × + Esc. **This is where the two carried modal defects die.**
  ⚠️ **The Esc defect was MIS-STATED and the code wins** (measured 10 Aug 2026 during F1a). It is NOT "`js/app.js:7503` + two parallel listeners". There is **ONE** listener (`js/app.js`, the `document` keydown near the backdrop-click wiring at the end of the file - grep for the id array, never the line number) that closes a **hard-coded list of 8** ids: `menuModal, invModal, confirmModal, editModal, delChoiceModal, builderModal, plateActionsModal, manageMenusModal`. Two OTHER Escape listeners exist and each closes exactly one modal (`#modal` via `closeModal`, `#kingWizModal` via `closeKingWizard`); a third closes the dashboard scope popover, not a modal at all.
  **So the real defect is the opposite shape to the one recorded:** because the list is fixed rather than "whatever is open", a genuine stack closes wrongly in BOTH directions - press Esc over a stacked confirm and every listed modal under it closes too, while **eight modals have no Esc handler at all** (`settingsPanel`, `ingModal`, `kingModal`, `smemModal`, `addDishModal`, `newMenuModal`, `tidyModal`, `tidyManageModal`). Real stacks exist and are deliberate (Settings→confirm for clear-cache and for restore; `ingModal`→confirm on product delete; `kingWizModal`→confirm on Add-all), and `#confirmModal` already carries `z-index:85` for exactly that reason.
  **Esc must close the TOP layer only**, derived from the DOM the way `syncBodyScrollLock` already derives the scroll lock (`.modal-overlay.open`) - not from a list. And **focus trap + return-to-opener must exist** (today nowhere in the app; the only return-to-opener anywhere is the dashboard scope popover). Build both in; don't drop them again.
- Motion: verify the shipped §1.4 set (entry rise + stagger, hover/press, shimmer) against the mock, add the sheet slide - all CSS, all behind `prefers-reduced-motion`.
...
**Do with: nothing.** F2-F10 are NOT blocked on this - they consume tokens, which shipped in F1a. Only work that needs the modal/sheet primitive (F8's upload sheet) genuinely wants F1b first.
...
- **The v3 opaque semantic tints no longer composite with row hover** (v132 review): `--warn-bg`/`--good-bg`/`--bad-bg` went from rgba washes to solid hex, so a tinted row (e.g. an invoice review row) fully masks `.atable tbody tr:hover td{background:var(--hover)}` — hover feedback disappears on exactly the rows being scanned. **F1a re-landed the tokens (v136) and did NOT resolve this** - the mock's own values are solid hex in both palettes, so the masking is unchanged and is now a dark-mode question too. **Re-pointed at F2**, the first F-screen to consume them (was "decide in V2", then "when F1 re-lands the tokens"): either hover wins via a composited overlay, or tinted rows deliberately don't hover. Note F1a DID separate `--hover` from `--surface-2` (they were the same colour before; the mock has #F2EADD vs #FAF7F1), so hover is a stronger wash now and the masking is more visible than when this was written.
