...
**Two live asks for Max**, both `blocked` below and both the same shape - a PROPOSED `CLAUDE.md` rule needing his yes to the rule and to its wording (work-item sequencing; measuring the fixed containing block). Neither blocks anything: a no costs nothing in both cases. (This paragraph read "No live asks for Max" until 10 Aug 2026, by which point both had been sitting here for a batch - a header that says nothing is waiting is how a `decide` file never gets written.)
