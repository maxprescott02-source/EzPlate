...
## next  The `.legacy` wrapper is described as operative and has never existed (AUDIT-v145 C1)
Problem: `docs/QUEUE.md:184` and `:455` both describe `FOLD-IN-PROTOCOL.md` §2's `.legacy` wrapper in the present tense as the mechanism scoping the old stylesheet to unconverted screens - and the Dead CSS sweep item is *relying* on it to "do most of this sweep structurally" later.
**`grep -rn "\.legacy" css/style.css index.html js/app.js` returns ZERO hits. The class does not exist and never has.**
`HANDOVER-151-menu.md` flagged exactly this and asked for a decision "before F6 rather than after F10"; **F6 shipped without one** and does not mention it. The manual per-screen deletion done instead has worked for six screens running, so nothing is broken - the prose is just wrong about how that safety is being provided, and one item is banking on it.
Requirements: decide once - either implement the wrapper on the four screens still unconverted (Builder, Invoices, Settings, Account), or strike the claim from both lines and record that per-screen manual deletion IS the mechanism. Then re-word the Dead CSS sweep item's `Do after:` reasoning, which currently promises work this mechanism was going to do.
Out of scope: the deletions already done, which were correct either way.

## next  The tint-vs-hover item still points at F5, which disproved its premise (AUDIT-v145 C2)
Problem: the *"v3 opaque semantic tints no longer composite with row hover"* line in the Small list still reads *"F5 (Menu) is: its food-cost verdict cell is the app's original tinted row."*
F5 shipped as `v142` and **deleted the entire `.atable` system**; its handover records that the masking concern was never about Menu at all. Menu's verdict is now a small `.vbadge` pill (`css/style.css:3637-3640`), not a full-row tint, and `.mnu-row:hover` (`css/style.css:3646`) is untouched by it.
So the item has now been re-pointed four times (V2 → "when F1 re-lands the tokens" → F2 → F5) and been wrong the last two.
⚠️ **The finding is the pattern, not the tint.** AUDIT-v135's C2 named this exact shape - a note routed at a future batch that the batch then does not answer - and it recurred in the same file, in the same phase, after being named. AUDIT-v145 D2 is a third instance. That is what to fix.
Requirements: re-point at the screen where a full-row semantic tint and hover genuinely coexist **today** - the invoice review's `.st-*` rows are the strongest remaining candidate, which makes it `Do after: F8` - or confirm no such row exists yet and say so. Do not re-point it a fifth time without checking the code.
...
Cause: `playwright.config.js` sets **no `retries` and no `workers`**. Retries default to 0 and the test timeout to Playwright's default 30s, while the CI runner takes ~7 minutes for a suite that runs in ~3 locally. One slow `browser.newContext` under load therefore fails the job outright.
⚠️ **The reason this is worth fixing rather than living with: a red check that is WRONG is the mirror image of the trap `CLAUDE.md` Tier 3 already documents at length** ("a green check has been wrong - and so has an absent one"). A spurious red is arguably worse, because it costs a re-run every time and trains the reader to dismiss red, which is exactly how a real failure gets merged.
Requirements: set `retries: process.env.CI ? 1 : 0` so a flake is reported as **flaky** rather than green (Playwright distinguishes them - a retried pass is not a silent pass, which is what keeps this honest). Consider `workers: process.env.CI ? 1 : undefined` if contention is the cause; measure before assuming it. Do NOT raise the global test timeout to hide it.
Out of scope: the spec itself, which is correct and passed on re-run - do not "fix" `v141-sync-corner.spec.js`, it is not what broke.
Note for whoever takes it: this is a config change, so it changes what runs and takes the mandatory pre-push review despite being two lines.
...
A one-line `CLAUDE.md` addition is proposed in the answers file so this is not asked a fourth time; it is NOT written yet and needs his yes.
...
> **Sequencing between pieces of work lives in `docs/QUEUE.md`, never in `CLAUDE.md`** - the queue re-checks its ordering every batch through the step-1 `Do after:` sweep, and `CLAUDE.md` has no mechanism that can notice a scheduling claim going stale.
The evidence is the sentence just deleted: "the dropdown placement work is therefore UNBLOCKED - the positioning context is already final" sat in `CLAUDE.md` after the decision that falsified it, and nothing could catch it. The `Do after:` field exists because the same rot in queue prose cost two years of versions.
⚠️ **The review also found the wording is TOO BROAD as written**, and that half is not a matter of taste: Tier 3's Migrations section legitimately states standing sequencing in `CLAUDE.md` ("Staging first, then production", "order the statements so the dangerous intermediate state cannot exist"). So a literal "never" contradicts rules the batch never touched.
Requirements if Max says yes: the rule distinguishes **which queue item runs before which** (belongs in the queue) from **standing procedure inside one piece of work** (belongs in `CLAUDE.md`). Then sweep `CLAUDE.md` for other instances of the first kind - this batch checked only the one the audit named.
Blocked on: Max's yes to a new Tier 2/3 rule, and to the narrower wording. A no closes it; the stale sentence is already deleted either way, so nothing is broken while it waits.

## blocked  PROPOSED CLAUDE.md rule: measure the fixed containing block, never `innerWidth`
Problem: proposed by the v141 batch (10 Aug 2026) after a geometry assertion passed on macOS and failed on the Linux CI runner three times running. Two fixes were attempted from theory without measuring and both were wrong; instrumenting the assertion to print its own geometry settled it in one run.
The proposed wording:
> **A viewport-geometry assertion must MEASURE the fixed-position containing block, with a `position:fixed;left:0;right:0` probe - never `window.innerWidth` and never `document.documentElement.clientWidth`.** On the CI runner all three disagree: the containing block is **370 inside a 380 viewport and 759 inside 768**, while `innerWidth` and `clientWidth` agree with each other and are both wrong. Anything centred with `left:50%` resolves against the block, so an expected-centre computed from `innerWidth` is off by half the difference - it passes on macOS, where overlay scrollbars make the three agree, and fails only in CI.
> The corollary bit twice in one batch: `.bottomnav` measures **370 wide at a 380 viewport**, so discriminating "left rail vs bottom tab bar" by comparing its width against the viewport misfires. Discriminate on ORIENTATION (a left rail is taller than it is wide).
Why it is worth a rule rather than a comment: every remaining F-item (F5-F10) rebuilds a layout and pins its geometry in Playwright, so this is not a one-off. It also has no natural home in the code - the next person to hit it will be writing a NEW spec, not reading the old one.
⚠️ **Check the scope of the wording before accepting.** It is stated as an absolute and the evidence is one runner and one browser; a narrower "when asserting a centred or percentage-positioned fixed element" may be the honest version. The v141 batch could not test a second CI environment to find out.
Blocked on: Max's yes to a new rule, and to the wording's breadth. A no costs nothing - the technique is already used and commented at `tests/visual/v141-sync-corner.spec.js`, so only the generalisation is at stake.
...
- **`.github/workflows/test.yml:174` count comment is stale, and drifting** - it says "9 specs, 8 survive"; actual is **22/21** (re-measured 10 Aug 2026, AUDIT-v145). It read 17/16 at v135, so the error has grown from 8 to 13 across ten batches in which nobody took this. **Flagged by two consecutive audits** - if a third finds it, the reason it keeps being skipped is the more interesting item. The guard is correct and fails closed; only the number lies. Workflow-file change, so it takes the mandatory review despite being a comment.
...
- **The v3 opaque semantic tints no longer composite with row hover** (v132 review): `--warn-bg`/`--good-bg`/`--bad-bg` went from rgba washes to solid hex, so a tinted row (e.g. an invoice review row) fully masks `.atable tbody tr:hover td{background:var(--hover)}` — hover feedback disappears on exactly the rows being scanned. **F1a re-landed the tokens (v136) and did NOT resolve this** - the mock's own values are solid hex in both palettes, so the masking is unchanged and is now a dark-mode question too. **Re-pointed at F5** (was "decide in V2", then "when F1 re-lands the tokens", then F2): **F2 shipped 10 Aug 2026 without consuming them** - the Plates screen has no semantically tinted rows, so it was never the first consumer and the pointer was wrong. ~~F5 (Menu) is: its food-cost verdict cell is the app's original tinted row.~~ ⚠️ **THAT WAS WRONG TOO, and F5 has now shipped and disproved it** (AUDIT-v145 C2): `v142` deleted the whole `.atable` system, and Menu's verdict is a small `.vbadge` pill (`css/style.css:3637-3640`), not a full-row tint - `.mnu-row:hover` (`:3646`) is untouched by it. **Do not read the pointer above as current; it has been wrong twice running.** The re-point is its own item now - see *"The tint-vs-hover item still points at F5"*. Either hover wins via a composited overlay, or tinted rows deliberately don't hover. Note F1a DID separate `--hover` from `--surface-2` (they were the same colour before; the mock has #F2EADD vs #FAF7F1), so hover is a stronger wash now and the masking is more visible than when this was written.
