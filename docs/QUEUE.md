...
**Problem:** the `.scr-sub` slot means four different things across eight screens; the Invoices subtitle **contradicts its own dropzone** ("Imports update product prices automatically" over "Nothing changes without your review"); "Recent imports" is a heading apologising for its own absence; Account and Settings carry roadmap notes.

**Requirements:** the subtitle is **scope or a count, never a sentence** — sentences move into the body. Fix the Invoices contradiction. Drop the "Recent imports" heading until the import-history feature exists (**item 87**). Move the Ingredients "164 of 399" line into the header sub. Remove the Plan card until there is a plan. Fix the four Settings copy slips, including **"Version vNN" doubling the v**.

⚠️ **The version line is printed from `APP_VERSION`, which already carries its own `v`.** Fix the concatenation, not the constant — six cache spots read that constant.
