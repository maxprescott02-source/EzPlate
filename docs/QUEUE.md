...
## next  The v3 sync/status treatment - spec'd in the package, owned by no F-item
Problem: found 10 Aug 2026 by the v141 batch, which fixed WHERE the sync banner sits and could not fix WHAT it is, because the item fenced off its copy and states.
The package designs this element twice and the app matches neither. §3.1's header row carries a quiet **"Synced 4 min ago"** at 12px `--text-3`, sitting inside the §2 header bar between the title spacer and the actions - in flow, not floating. §5/§72 designs the error case as a **red-tinted in-flow banner** (border `#F0D4D1`, bg `#FBEBEA`) with the message and a **Retry** button, "persistent until resolved, never auto-dismisses", and names it as mapping to the shipped `#syncBanner`.
The app instead has ONE floating pill for all five `setSync` states, phone-shaped, with no Retry and no in-flow position at any width.
**Why no F-item catches it:** the package's own build order (§118) makes "States (§5) wired to `#syncBanner` and `toast()`" step 8, a shell/states pass - it is not a screen, so F5-F10 each restyle their screen and none of them owns this. Same shape as the More-screen gap, and worth saying out loud for the same reason: without it §5 is unmet by construction when the phase is called finished.
