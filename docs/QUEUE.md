...
**Conversion state is read from THIS FILE'S F-items and nowhere else** — not from the paint. F1a-F9 have shipped (`ezplate-v136`-`v143`, F7 as `v146`, F8 as `v147`, F9 as `v148`); F10 alone remains. There is no reset pass and no clean starting line (Max, 10 Aug 2026, overriding §0a): §2 binds FORWARD.
...
## doing  1 · F10 — Account (desktop §3.9 + mobile §6, one item)  **[B]**
...
**ANSWERED (Max, 11 Aug 2026): ship the minimal screen, and fill it out during multi-tenant.** He did NOT take the recommendation, which was to leave Account to the multi-tenant phase entirely. His call, made — do not re-litigate it, and do not propose deleting the screen in a later tidy.
The shell ships the mock's three §3.9 sections (Profile, Team, Plan) in the mock's order so multi-tenant fills them in place, and NONE of the mock's controls: Edit profile, Invite a teammate, Manage billing and Sign out each have nothing behind them, and §R4 forbids a control that does nothing. **A shell is not a licence for a dead control.**
The Account and Team cards move off Settings onto it, and one Settings row becomes its only route at any width; the More-screen item adds the second.

## next  2 · The plate builder — restore the FILL ORDER the redesign lost  **[B]**
...
**Three of the four now are** — Products (F4), Invoices (F8), Settings (F9). Account is F10's, which is what `Do after: F10` is waiting for.
