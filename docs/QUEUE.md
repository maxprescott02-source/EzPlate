...
### The §5 contract, MEASURED 11 Aug 2026 — read this before touching the screen

Done as read-only investigation at the end of the F8 batch so it is not redone. **§5 requires this list before the screen is touched; it is that list.**

**The routing fact that decides the whole item, and it is not what the markup suggests.** `#settingsBtn` (the header gear) looks like it is always there. It is not: `css/style.css` hides `header{display:none}` inside `@media (min-width:1024px)`, so **the gear is MOBILE-ONLY (<1024) and `#sideSettings` is DESKTOP-ONLY (≥1024)**. Two routes, no overlap, no width without one.
**Consequence: Settings can become a real `#tab-settings` screen NOW, and it does not wait for the More screen.** Both existing entry points simply navigate instead of opening an overlay, and neither disappears. That is a better position than F8 inherited — Invoices had no phone route at all and Settings has one. **Do not remove either route.** The More-screen item later adds a third; it does not replace these.

**Every handler and control, so none is discovered by deleting it.** Panel: `settingsBtn` · `sideSettings` · `settingsClose` · `settingsDone` · `settingsBack` · the seven `.set-navitem[data-goto]` (general, invoices, lists, data, account, team, about) driving `setSettingsSection(id, drill)`, whose `drill`/`.detail-open` mobile list→detail behaviour is a MODAL affordance and is exactly what a screen replaces.
Controls, all read or written by `openSettings`: `setCogsInput` (target %) · `setGstDefault` · `setVersion` · `setAiInvoiceChk` · `setAiSuggestChk` · the theme segment `setThemeLight`/`setThemeDark`/`setThemeSystem` + `syncThemeSeg()` · `setSmemOpen` · `setTidyOpen` · `setExport` · `setRestore` + `setRestoreFile` · `setClearCache`.
⚠️ **`openSettings` PRIMES the form on every open** (`cogsPct`, `gstDefault`, `APP_VERSION`, both AI flags, the theme segment) and then `setSettingsSection('general', false)`. A screen has no open event, so **the priming has to move to the screen's render** or every value renders stale. This is the single most likely way to break the screen silently.
⚠️ `#settingsPanel .settings-panel` is named in the shared sheet/modal CSS at the end of `css/style.css` and in the `max-height:calc(88vh - 40px)` rule. Both come out with the modal, and both are shared selectors — grep before deleting.

**What the mock's §3.8 actually lists**, against what exists: Business (name, currency) — **R4, neither exists**. Costing (target food cost ✓, "Prices include GST" — the app's `setGstDefault` is a two-option select, not a boolean, and it means "assume when the invoice does not say", so R2: keep the select's meaning). Notifications (price-rise alerts, weekly summary) — **R4, no notification system, no email, no scheduler**. Data (export everything as CSV — **R4, the maintenance behaviour spec**; Delete workspace — **gated on multi-tenant, do not ship**). The mock also puts **Sign out** and a **Save changes** button in the header: R4 (no auth) and R2 (this app saves on change, so a Save button would be a control that does nothing).
**The app has four sections the mock does not:** Invoices (GST default, AI invoice check, Remembered packs), Lists (Tidy lists), About (version, contact, the privacy sentence), and the Theme segment. All are live and load-bearing — **R3: rehome them, never drop them.** The Account and Team sections are already honest one-line placeholders; F10 owns whether they stay.
...
# Multi-tenant phase — items 6-14 above are its gates
