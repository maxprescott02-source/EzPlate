...
✅ **BATCH 202 CLEARED THE TOP SIX** — `invGstDetect`, `costAtLines`, `unitCatCategory`, `derivePackPrice`, `costFromLines` and `analyze` are targets now. Twenty-two survivors: nineteen killed by assertion, three allowed with **enumerated** proofs of equivalence rather than arguments that they are unlikely. **99 survivors remain across four functions in THIS item, and every one of them is 12 or more** — the cheap slice is finished and the next one is a different size of job. *(A first draft said 143, which is this item's 99 plus `gemApplyReadings`'s 44 from item 0c2 — a real total of the mutation debt, but not of this item, sitting above a table that sums to 99. Caught by the pre-push review. The two items are counted separately on purpose: they are separate items.)*
...
✅ **BATCH 204 CLEARED `resolveMatchedPrice`** — twenty-four survivors, twenty-one killed by assertion in the new `tests/matched-price.test.js`, three allowed. 55 mutants, 31 killed before the batch and 52 after. **Only `computeInsights` remains in this item.**
⚠️ **ITS `tests:` LIST GREW RATHER THAN BEING REPLACED, which is the opposite of what 203 did to `applySupplierMemory`, and the difference is evidence rather than taste.** The four files already named there DO exercise it — 31 of its 55 mutants died against them before this batch — so they are kept and the new file is added. Measured both ways: with the new file alone one mutant survived that the old four kill, which is what settled it. (That mutant is now killed directly too, so the four are currently redundant FOR THE GATE and remain honest as a list. Twelve seconds a run.)
⚠️ **All three allowances are the same `>` -> `>=` shape on a positive-quantity guard, and that makes FIVE in this file.** The pattern is worth naming so the next slice recognises it instead of re-deriving it: **the money path is written defensively, and defence in depth is exactly what produces equivalent mutants on the outer layer** — a later guard already refuses everything the loosened one lets past. It is a pattern, not a proof; each was still enumerated. One of the three is stronger than equivalent and worth reading before copying: `q>0` inside the memory arm is the SAME expression the guard ten lines above already required, so it is fully redundant rather than merely harmless.

⚠️ **AND THE RUNTIME PARAGRAPH THIS ITEM CARRIED WAS WRONG WITHIN A DAY — MINE, NOT SOMEBODY ELSE'S.** 203 wrote down 112s on `main` and 114s here and told the next batch not to trust a figure it had not taken. 204 took it: **the same 538-mutant configuration measured 270s**, on the same laptop, twenty-four hours later. Nothing about the repo changed between the two.
**So the absolute figure is not a property of this repo and must stop being recorded as one.** 201's 306s, 202's thirteen minutes and 203's 114s are all real measurements of a machine, and each was written down as a fact about the gate.
**What IS stable is the MARGINAL cost of adding a target, measured back to back in one sitting: 55 mutants for about 14 seconds.** That is the number a batch actually needs, it is cheap to take, and it does not rot. **If you need to know whether the gate is too slow, time it twice yourself in the same hour; do not read it here.**
...
|---|---|---|
| `computeInsights` | 39 | plus the one that hangs. Only `insight-coverage.test.js` runs it; `settings-toggles.test.js` greps its SOURCE for an ordering and will contribute no kills |
...
Requirements: the line above reaches zero survivors and moves from `pending` into `targets`. **This item is then DONE and is deleted** — `gemApplyReadings` is item 0c2 and is not part of it.
⚠️ **`computeInsights` is not another slice of the same job, and three things say so.** It is the only remaining function with the mutant that HANGS rather than fails, so it must not be attempted on a gate without 201's per-mutant bound. It is the impure Dashboard builder rather than a pure pricing helper, so its fixtures are whole app states (`MENU`, `savedPlates`, `PRODUCTS`, both price logs) rather than a row and a pack. And 39 is the largest count this item has ever had in one function. **Expect it to be a batch, and expect the fixtures to be most of it.**
