...
## next  Playwright / Chromium bump - the only actual fix for the segfault
Problem: the crash diagnosed 10 Aug 2026 is in `chrome-headless-shell-1228` shipped with Playwright **1.61.1** (`package.json` pins `^1.61.1`). It is a null dereference at a fixed code offset, so it is a browser bug, and the only real fix available to this repo is a different browser build. Retries hide it competently in the meantime, which is why this is not urgent.
...
Note this cannot be shown to have WORKED, only to have not recurred: the measured rate is **5 CI occurrences across the 42 `tests` runs of 10 Aug**, so a handful of green runs proves nothing. Record the version and the date, and treat a later occurrence as evidence against the bump rather than as a new mystery.
...
- **The `v141-sync-corner` setup failures - a Chromium SEGFAULT, and the file was never the subject** - answered 10 Aug 2026, **docs + one CI comment, no deploy version** (`sw.js` stays `ezplate-v145`), handover `HANDOVER-163-browser-segv.md`.
  **THE CAUSE, measured, not inferred: the browser PROCESS CRASHES.** All **five** CI occurrences carry `Received signal 11 SEGV_MAPERR 0000000001b0`, and **all five crash at the IDENTICAL instruction, binary offset `0x2af9eec`** (module offset subtracted from the absolute address, so it survives ASLR), in `chrome-headless-shell-1228` under Playwright 1.61.1. `ax` is 0 and `cr2` is `0x1b0` - a null dereference at a fixed member offset; `rcx` holds the ASCII `ocalhost`. It is one bug, five times, not a family of crashes. `--disable-dev-shm-usage` is already on the launch line, so the usual /dev/shm explanation is excluded. **Nothing in the spec's own code is involved** - the item's observation that the file manages no contexts was right, and this is why.
  ...
  ⚠️ **THE PREMISE "all in this one file" IS FALSE, and this is the half that matters.** A census of **all 42 `tests` runs of 10 Aug** - branch runs included, they are inside the 42, not additional to it - found **six** occurrences, not three, in **two** spec files (five in CI, one local):
  `31348972108` (branch, 02:08) `v141:90` @1440 · `31349737666` (branch, 02:25) `v141:90` @1280 · **`31359764333` (main, 05:48) `v142-menu.spec.js:67`** · `31368954663` **attempt 1** (branch, 08:11) `v141:174` · `31380462471` (PR #144, 10:44) `v141:125` · plus the local `:229`, same error string, no log kept.
  ...
  **THE ARITHMETIC, since the item asked for it - and it does NOT come out as cleanly as "chance".** Two of the six are not independent samples: both branch-run occurrences come from runs where two OTHER `v141` tests were failing for a known unrelated reason (the ICB-geometry assertions that batch was fixing), and **Playwright discards the worker and launches a fresh browser after EVERY failure** - evidenced in this repo's own report, where PR #144's worker 1 died and the retry ran on a worker 2 that had not existed before. The local one has unknown sampling; what was run during that rehearsal is recorded nowhere.
  **That leaves three clean CI occurrences: `v141` twice, `v142` once.** `v141` is 12 of 209 tests (5.74%), so two-or-more of three landing in it is about **1 in 105** - a long way from the item's implied 1 in 5,400, and much too weak an n to call a defect. **Recorded as UNEXPLAINED, not as chance and not as cause**, because the honest state is that the only testable mechanism was refuted:
  **the lead that DIED, written down because it was the only substantive one.** Fastest context churn was the candidate - crashes land at context boundaries, so the file making the most per second should meet the most. `v137-modal-layer.spec.js` runs **22 tests at a 1215ms mean** against `v141`'s **12 at 2235ms**: it creates and destroys contexts nearly twice as fast and has never crashed once. Refuted by its own control.
  ⚠️ **Do NOT reopen this as "why that file".** Whatever tilts the odds, the failure is a browser bug and the response is a retry either way; the two follow-ups below are the only work it can generate. If a future occurrence lands in a third file, that closes it - **so record the file every time one appears.**
