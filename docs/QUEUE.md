...
⚠️ **`app_settings` is no longer theoretical, and the failure was MEASURED on staging in 182, not reasoned:** with the tenant policies live, a second café restoring a backup that contains `app_settings` is refused **42501 on the USING expression**, because `key` is the primary key and is global — the colliding row belongs to another tenant and is invisible, so the upsert cannot see what it collides with. It fails LOUDLY, which is the acceptable version of this bug, and it is the concrete cost of leaving this item unfixed: **a second café cannot restore a backup at all.** Build the composite key (`business_id, key`) and this goes away; the same measurement is in the migration's header under REHEARSED.
