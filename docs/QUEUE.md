...
# Phase law — the v3 fold-in (F7-F10)
...
**Conversion state is read from THIS FILE'S F-items and nowhere else** — not from the paint. F1a-F6 have shipped (`ezplate-v136`-`v143`); F7-F10 remain. There is no reset pass and no clean starting line (Max, 10 Aug 2026, overriding §0a): §2 binds FORWARD.
...
## next  1 · F7 — Plate Builder (desktop §3.7 + mobile §6, one item — FULL PAGE)  **[B]**
...
**Ships the `CLAUDE.md` Tier 2 edit**: "builder IS a MODAL" → the full-page decision, dated, with the full reversal history. The 9 Aug reversal STANDS.

*Two behaviour specs it must answer here rather than route onward (R4 — visibly disabled with a reason, or absent; never decorative):
- **Duplicate.** Trigger: the Duplicate button. Data: lines + category cloned into a new UNSAVED plate; name rule to decide. State: publish state NOT copied (a copy published to the same menu would duplicate a dish); interacts with `guardUnfinishedPlate`. Error: the unsaved-work guard fires before the clone, not after.
- **Recent range on the cost card.** Trigger: cost card render. Data: read-only derivation from `priceHistory`; window and copy to define. State: display only. Error: fewer than two points renders nothing, never "$X to $X".

## next  2 · F8 — Invoices (desktop §3.6 + mobile §6, one item)  **[B]**
...
## next  3 · F9 — Settings (desktop §3.8 + mobile §6, one item)  **[B]**
...
Mobile: sub-screen under More, back chevron. (**Do with, or immediately after, item 5** — the More screen is what gives this a mobile home.)

## next  4 · F10 — Account (desktop §3.9 + mobile §6, one item) — expect this to reduce to ONE QUESTION  **[B]**
...
## next  5 · The mobile More screen, and Products/Invoices/Settings/Account as sub-screens under it  **[B]**
...
## next  6 · One action in a mobile screen header — rehome the second one (DECIDED 10 Aug 2026)  **[B]**
...
## next  7 · Unique ID generation  **[A — launch blocker]**
...
## next  8 · Staging Supabase — mirror the schema and seed it  **[A — the safety net for items 9-11]**
...
## next  9 · Supabase Auth  **[A — launch blocker]**
...
## next  10 · `business_id` on every table, plus RLS  **[A — launch blocker]**
...
Do after: **8** (staging) — this is the largest unrehearsed migration in the project.

## next  11 · Roles — owner vs staff  **[A — launch blocker]**
...
Do after: **10** — roles are enforced in the same policies.

## next  12 · Onboarding and empty states  **[A — launch blocker]**
...
Needs item 8 (staging) to test at all.

## next  13 · The privacy gate  **[A — launch blocker]**
...
## next  14 · pdf.js 4.2.67+  **[A — launch blocker]**
...
## next  15 · Gate review before public signup  **[A — launch blocker]**
...
Do after: **10, 13, 14** — it is the read-through of the gates, not a substitute for them.

## blocked  16 · The restore's full-wipe step (step 3)  **[A — data integrity]**
...
**SCHEDULED (Max, 9 Aug 2026): runs when the v3 fold-in phase finishes (items 1-6), before any multi-tenant work.** The batch that closes the phase prepares everything and asks for the go.
Blocked on: Max's go on the day. The timing question is answered; this is not an open ask.

## next  17 · Floating layers and mobile dropdowns  **[B]**
...
## next  18 · Desktop shell polish — nav hierarchy, rhythm, and the full-bleed header  **[B]**
...
## next  19 · Products table polish — five measured defects, one screen  **[B]**
...
## next  20 · Dashboard trend polish — the x-axis and the third accent hue  **[B]**
...
# Multi-tenant phase — items 7-15 above are its gates
