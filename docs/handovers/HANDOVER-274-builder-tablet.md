...
**The batch made the same mistake it was fixing.** Item 49's figures were measured on one state and generalised; so were mine, three times - "the desktop is unchanged" from 1360 alone, the save bar's rail clearance from 1024 alone, and the wrap point from macOS alone. None was caught by me.

**The third is the one worth carrying, and CI found it after the review had passed.** The wrap point is where `.bld-body`'s inner width reaches 800, and that depends on the **scrollbar** as well as the viewport - overlay here, classic on the Linux runner - so 1076 is two-column on this machine and still wrapped on that one.
⚠️ **It was not a flaky test. A media query pinned one pixel past the locally-measured wrap gives every classic-scrollbar machine a band where the rule says "two-column", the bar hides and `#saveBtn` returns to a rail still under the docket** - a width with no reachable commit, which is the defect this batch exists to remove, reintroduced by its own fix.
**And the gate that missed it is the instructive part:** `tests/builder-readiness.test.js` asserted the three new media queries agree with **each other**, and they did, perfectly, at 1075/1075/1076. **Agreement between rules is not agreement with the layout.**
The threshold is now 1100 and deliberately loose, because the two errors are not symmetric - the bar shown slightly past the wrap is redundant, the bar hidden slightly before it is broken. The spec sweeps 900-1200 and asserts the **property** (wherever the rail wraps, a commit control is on screen; never two, never none) instead of naming a width, and the unit test now asserts the **asymmetry** with a stated scrollbar allowance.
**`.claude/rules/tests.md` already says a viewport-geometry ASSERTION must measure its reference rather than name it. This batch is the evidence that the same is true of a RULE** - and unlike a test, a wrong breakpoint ships.
