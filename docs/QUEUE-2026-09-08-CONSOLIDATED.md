...
## next  90 · The other three halves of item 21: a completion message, a delete sequence, and a boot-time replace  **[B, unmeasured; split out of 21 by batch 247, which shipped its history half]**

**These are 21's remaining instances, carried forward verbatim in substance.** They are grouped because 21 grouped them, and they are apart from the history gating because that one shipped. **Each needs its own repro; none of them falls out of the others.**

- **Invoice Apply announces completion before the writes settle** (`applyInvoice`, grep it — the line number 21 carried was `:12065` and has moved twice since): the applied count increments synchronously, bookkeeping is written, the dialog closes and "Invoice imported" renders without awaiting the product upserts. Later failures toast, so it is a false completion rather than silent loss. **What must be true:** the completion message waits for the upserts and says how many landed; a rejected write leaves the row unticked with its error. **Test:** `inv-unit-rebase-apply.test.js` gains a case whose writes reject after Apply.
  ⚠️ **This is also where the all-menus trend point for an invoice belongs.** 247 left `logHistory()` ungated on that path on purpose: one call stands for dozens of product writes with no single verdict, and the honest gate is the same manifest this bullet needs for its count. Do them together.
- **`doDeleteMenu`'s unawaited dish deletes**: the change-log entry chains off the write that decides the menu is gone, and the dish deletes are fired without being awaited. No FK to violate (`menu_items.menu_id → menus.id` is ON DELETE SET NULL), so this is the v112 sequencing class rather than a 23503 risk. The comment at the site already says it is out of scope for the batch that wrote it.
- **`priceHistory` wholesale replace at boot**: an empty or filtered server response replaces local, while `menuHistory` merges. A point logged offline is lost at the next sync. Make it merge like its siblings or say at the site why not.
  ⚠️ **Note the interaction 247 creates:** with the trend point now gated on the write, a point that exists locally is one whose write was ISSUED and accepted, so the offline case this bullet is about is narrower than it was — but not gone, because `pushWrite` still drops writes when fully offline and toasts rather than queueing.

⚠️ **`Do after:` NONE, deliberately.** 21 carried one that named the fictional menu from item 20 as its deterministic failure trigger, and 246 removed that state entirely. **Each of these three needs a trigger of its own** — a refused write injected at the Supabase client is the copyable one (`tests/visual/246-menus-read.spec.js` does it for a table; `tests/history-paths.test.js`'s `fail` option does it per write kind).
