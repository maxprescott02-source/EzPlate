...
## next  41 · Nothing records that a user accepted the privacy notice  **[B since 25 Sep 2026 - the pilot puts a stranger through the sign-up form; it was C, due to become B the day 2b ships]**
...
## ~~47 · One modal footer pattern~~  **FINISHED. Four bullets in batch 272 (`ezplate-v221`), the builder's Clear/Delete pair in batch 273 (`ezplate-v222`).**  **[B, the same intent sits in three positions and two red-adjacent buttons on one screen mean different things]**

✅ **THE BUILDER'S PAIR SHIPPED, batch 273.** `#clearBtn` reads **"Start over"** and its two `--bad` rules are deleted, so `--bad` belongs to `#bldDelete` alone on that screen.
⚠️ **THE DEFECT WAS THE COMPUTED COLOUR, AND IT WAS EXACT RATHER THAN MERELY SIMILAR.** Measured in Chromium on a saved plate before the fix: `#clearBtn` and `#bldDelete` returned **the identical string** — `rgb(192, 57, 47)` light and `rgb(229, 135, 125)` dark — and at 1360×900 both were painted at once, at y=279 and y=681. The item called them "red-adjacent"; they were the same colour.
❌ **"MOVE IT INTO THE DOCKET HEADER" WAS NOT DONE, DELIBERATELY, AND THIS IS THE RECORD SO IT IS NOT RE-PROPOSED.** Two measurements, both at 380:
  - there is **no element called the docket header**. The nearest is `.bld-mast`, 322px of 11px letter-spaced mono with `white-space:nowrap;overflow:hidden`, already holding a title and `#dCount` hard right. A 44px control there clips one of them or breaks the touch minimum batch 272 had just finished restoring on `.use`;
  - the two verbs were **402px apart already**. Distance was never what made them read alike — the colour and the `<verb> plate` grammar were, and both are fixed.
  `tests/visual/273-builder-discard.spec.js` asserts the non-move at 380 **together with the reason for it**, so a later batch reading this bullet cannot take the move without the test telling it what it is spending.
⚠️ **A THIRD REASON WAS WRITTEN HERE AND WAS TRUE OF ONE PLATE STATE ONLY. 273's PRE-PUSH REVIEW CAUGHT IT, AND IT IS LEFT WRITTEN OUT RATHER THAN QUIETLY DELETED.** It read: *"below 768 `.bld-actrow` is the only thing `#bCost` still shows, so moving Clear out leaves a bordered card containing one button"*. **That is true of a plate on NO menu and false the moment the plate is published**, because `.is-bare` — and with it `#bCost .bld-cardbody{display:none}` — is set from `on.length===0`. Re-measured at 380 across all three: **card height 60px / 127px / 164px** for 0 / 1 / 2 menus, the second carrying `#bPriceRow` and the third `#bMenus`.
**The failure was the fixture, not the reasoning:** the probe that produced the number seeded a plate with no dish rows, and the result was written down as a fact about the screen. The stylesheet had the counter-evidence in the same media block the whole time — `#bCost .bld-menus{border-top:0;padding-top:0}` is only worth writing if that list renders at this width. **`.claude/rules/app-guards.md`'s "an exemption is scoped to the CLAIM that justified it" in its measurement costume: one seed is one state, and a claim about a screen needs the state named.** The spec now runs the 380 case as a TABLE over 0, 1 and 2 menus and asserts the bare-card case by its condition.
✅ **"Keep Delete alone at the bottom" was already true and needed no change:** `.bld-actions` holds `#bldDuplicate` (neutral) and `#bldDelete` (`danger ghost`). Delete is the only destructive control there, which is what the bullet meant.
⚠️ **AND ONE THING THIS BATCH MADE QUIETER WITHOUT MAKING IT SAFER, raised as its own queue item rather than fixed here:** the discard has **no confirm and no undo**, and `clearPlateDraft` does `removeItem(DRAFTKEY)` — so on an UNSAVED plate the work is gone and `guardUnfinishedPlate` cannot offer it back. Removing the red removed the only "this is heavy" signal it had. The item says *"keep the behaviour"* and it is kept; the question of whether it should ask is filed in `docs/QUEUE.md`.

✅ **Shipped: the footer placement, the Done buttons, `.del-link`'s retirement and `.use`'s touch target.** One rule places every edit form's destructive action, `.mfoot > .btn.danger:first-child{margin-right:auto}`, replacing three separate mechanisms. `tests/modal-footers.test.js` and `tests/visual/272-modal-footers.spec.js` are new.

⚠️ **THE ITEM SAID "ONE PATTERN" AND THERE ARE THREE SHAPES, which is the correction worth carrying to 53 and 54.** Eighteen modals have a `.mfoot`, not the three named. An EDIT FORM puts its destructive action far left as an escape hatch; a CONFIRM dialog's destructive action is its PRIMARY and belongs on the right (`#delChoiceModal`'s "Delete everything"); a DISMISSAL has one button that only closes. Collapsing them to one rule would have moved "Delete everything" to the left of its own dialog, which is why the shipped selector is keyed on `:first-child` and not on `.btn.danger`.
⚠️ **Two more counts were short.** Five dismissal buttons in FOUR styles, not the two named (`privacyDone` also says "Close" rather than "Done", kept deliberately: the other four end a piece of work and a notice is read, not worked in). And `#ed_delete` was an `<a role="button" tabindex="0">` with a click handler and **no keydown**, so Enter and Space did nothing on it — an accessibility defect no audit had named, fixed for free by making it a real `<button>`.
...
- ~~**"Clear plate" beside "Delete plate" (U11):**~~ ✅ **SHIPPED, batch 273, `ezplate-v222`** — renamed and de-reddened; the move into the docket header was measured and declined, per the strike at the top of this item. `#clearBtn` (`index.html:451`, `css/style.css:462` paints it `--bad`) empties the docket; `#bldDelete` (`:474`) removes the plate. Rename Clear to "Start over", move it into the docket header, drop its red, keep Delete alone at the bottom. ~~`js/app.js:9645`'s comment records why Clear exists (the app's explicit discard); keep the behaviour.~~
  ⚠️ **THIS BULLET WAS THE ONLY PART OF 47 STILL OPEN. Batch 272 shipped the other four and deliberately left this one**, because it is BUILDER-SCREEN work rather than modal-footer work and the Design law forbids mixing the two in one change set. It is not a half-finished job; it is the half that belongs to a different screen.
  ⚠️ **273 FOUND ONE MORE LINE NUMBER WRONG IN 272's OWN CORRECTIONS, WHICH IS WORTH RECORDING AS THE THIRD GENERATION OF THE SAME DRIFT:** 272 corrected the `--bad` paint to `css/style.css:542-543`; it was at **550-551**. Every other correction 272 measured held exactly. **A corrected citation is still a citation, and the file moves under it the moment anyone edits above it** — which is the argument for greping the symbol rather than for correcting the number a third time.
  **Corrections measured by 272, so the next batch does not re-derive them:** `#clearBtn` is at `index.html:479`, not 451, and `#bldDelete` at `:502`, not 474. The `--bad` paint is `css/style.css:542-543` (`#clearBtn{color:var(--bad)}` plus a `--bad-bg` hover), not 462, which is a section-header comment. **The comment recording why Clear exists is at `js/app.js:2668`** (*"v82: explicit discard clears the draft. F7 (v146): it also drops loadedPlateId…"*), not 9645, which is inside the team-invite deletion block and unrelated.
  ⚠️ **AND THE SCREEN MOVED UNDER THIS BULLET IN BATCH 271, which is the thing to re-check before planning.** `#clearBtn` and `#printBtn` now live in `.bld-actrow` inside `#bCost`, both are DISABLED by `syncBuilderPlateActions` when there is nothing to act on, and on a phone `#bCost` carries **only** those two buttons — everything else in the card is hidden below 768. So "move it into the docket header" removes one of the two things that card holds on a phone, and the `is-bare` rule that hides the empty card body was written against the card as it stands. Re-measure at 380 before moving anything. The rename itself is free: `#clearBtn` is NOT in `tests/verbs.test.js`'s table, so "Start over" collides with nothing.
...
| 1076 | 800 | no | 280 | - | 140 (was 70 before the fix) | on screen | not painted |
...
- **The band is 768 to 1075, not "1024 and 900".** Three of the widths a laptop opens the app in, not two.
- **THE WRAP POINT IS EXACTLY 1076, not "about 1120"** - at 1076 the container is 800, which is `500 + 280 + 20` to the pixel. ⚠️ **It was 1056 when first measured, against the SHIPPED `.bld-main` basis of 480; batch 274 raised that basis to 500 as part of the fix (480 split the columns at a docket the five-track row cannot draw), which moved the wrap point with it. Both numbers are correct about different code, and 1076 is the shipped one.**
...
- **`#bldSaveBar` is not painted at ANY width in this band**, so from 768 to 1075 there is no save control on screen at all once the page is scrolled. The item said "at 900 Save plate is below the fold"; the true statement is that the whole band has no reachable commit.
...
**So this needs no container query** - and that matters beyond tidiness, because `container-type:inline-size` establishes a containing block for `position:fixed` descendants, and the ingredient dropdown is a `position:fixed` descendant of `.bld-body` placed by `anchorDrop` from `getBoundingClientRect()` numbers. `.claude/rules/css.md` is an entire section about that exact pairing. **A plain `@media (min-width:768px) and (max-width:1075px)` is the wrapped state precisely, and carries none of that risk.**
⚠️ **The wrap point IS coupled to the sidebar's width and that coupling must be PINNED, not just written down** - if the sidebar changes width the wrap point moves and the query is silently wrong, which is the fragility the site comment was right about even though its conclusion was not. Assert that the rail is wrapped at 1075 and side by side at 1076.
...
**Add to the test list, from the re-measurement:** the wrap point itself, asserted from both sides (wrapped at 1075, side by side at 1076), because it is coupled to the sidebar's width and nothing else would notice that moving.

## ~~50 · Toasts land on controls: "Loaded" on the builder discard and the misc input, any toast on the builder's Save, a toast on a bottom sheet~~  **SHIPPED, batch 275, `ezplate-v224`**

✅ **All three bullets, on the one mechanism this item named — and it was right about the mechanism and short about the scope in all three, which is this file's usual direction.**
- The `Loaded: <plate>` toast is deleted. The item cited `js/app.js:9475`; the function was at **11384** and `:9475` is `authApply`. The sibling it told the batch to keep is **`openHealedPlate`**, not `loadMenuItem`, which does not exist — `:9412` is comment prose about `APP_VERSION`.
- `.bld-bar` publishes `--bld-bar-clear` and the toast takes the max. ⚠️ **The item states the publisher as an existing fact; it did not exist** — `docs/MAINTENANCE.md` said *"`.bld-bar` **would** publish"* and this entry dropped the conditional. Found by the premise check before any edit.
- ⚠️ **AND THE OVERLAP IS AT EVERY WIDTH THE BAR IS SHOWN AT, not the one the item measured.** Measured at 800px tall with a costed plate: toast y616-708 against the bar at y635-735 (380, covering `.bfs-save`), y700-800 (768 and 900), y701-800 (1024). Above 768 it clips the "Plate cost" label rather than burying the button — same defect, quieter symptom, and a fix judged against 380 alone would have read as complete.
- **The sheet bullet resolved to something narrower and sharper than "toasts on sheets".** Nineteen of the twenty sheets are ALREADY clear: a `.mfoot` is 76px against the toast's 92px dock. `#delChoiceModal`'s footer **stacks three choices and is 127px**, so the toast landed on the buttons of the delete-choice dialog — one sheet in twenty, and the highest-stakes one. A third publisher, `publishSheetFootClear`, on the same max(); the other nineteen do not move, and a test pins that they do not.
⚠️ **What the lift does NOT do, stated because the next reader will assume it did:** it does not make a sheet toast-free. At 380 the tallest sheet has 96px of headroom and a real toast is 91.5px, so no position clears a sheet entirely — the lift moves what the toast covers from the footer's CONTROLS to the dialog's title, which is the right trade and is not the same as solving it.
**A fourth pair came out of photographing the fix and is queue item 97:** `.sync-banner`'s error state sits on `#bFootFigs` at 1024-1099.
...
## ~~51 · Builder readiness and copy: Save is live on an empty plate, Print and Clear on an empty docket, an "Editing:" line in the accent colour, a lower-case helper, a grammar slip, and a duplicate line with no warning~~  **SHIPPED WHOLE, batch 271, `ezplate-v220` — five bullets of six, and the sixth DOES NOT REPRODUCE**

✅ **Taken in one PR rather than split.** The queue entry flagged that three of the six bullets are readiness BEHAVIOUR rather than copy and allowed the copy half alone; the behaviour half turned out to be three small functions on one screen, so there is no residue to write back.
`builderSaveBlocker` is the one source for "why not yet", `syncBuilderPlateActions` grew from two controls to six and owns every one, and the sentence renders into two sinks with one writer each — `#saveHint` in the rail and `#bFootLine` in the phone's sticky bar, which hide and show at the same breakpoint as the Save button each sits beside.

⚠️ **THE GRAMMAR BULLET DOES NOT REPRODUCE AND NOTHING WAS CHANGED TO MAKE IT TRUE.** `js/app.js` pluralises **both** halves — "1 item **has** no usable cost and **is** not in the total", "3 items **have** … **are** not" — and has since batch 245 widened that wording for negative costs. The item cited `:1882`, which is prose inside a block comment. That is this file's own rule about line citations arriving for the fifth time.

⚠️ **AND TWO OF THE FIVE WERE WRONG ABOUT THE REMEDY, not just the address, which is worth reading before running 52-56 off the same audit:**
- The Cost card is **60px** at 380, not the 16 both this item and `docs/MAINTENANCE.md` record, and `#bCost > h2` does not exist. 177 rebuilt it; what is left in the card is Print and Clear. "Hide the card when it has nothing to show" would have deleted two controls from the phone (§R3). The `.bld-cardbody` inside it is what is empty, and that is what hides.
- "Merge into the existing line's quantity" is the wrong half of this item's own either/or: a new line carries `defaultQty(p)` or null, so merging adds a number the user never typed to one they did, silently, on a costing screen. The duplicate is REFUSED instead — a toast naming the ingredient, and the cursor put in the existing line's quantity.

**`premise-check` found 4 of 6 line citations wrong** (`syncBuilderPlateActions` 11503 not 9650; `updateTotals` 2380 not 1873; the helper 431 not 412; the add path 2124-2210 not 1652-1670, which is `bootstrapSync`'s tenant gate). Every behavioural claim but the grammar one held.
New: `tests/builder-readiness.test.js` (22 tests). `tests/visual/v150-builder-order.spec.js`'s unnamed-plate spec is REWRITTEN rather than deleted — its guarantee was "the refusal brings the field to the user", and a disabled Save has no press to make, so the reason is printed in the always-visible sticky bar and the line itself is the route. `builderSaveBlocker` and `addKitchenLine` join the mutation gate; `syncBuilderPlateActions` gains a third test file.
...
## ~~53 · Edit modals hide the number the user is there to react to: the menu item form shows no cost, the product form changes the unit of thought, and money fields do not look like money~~  **THREE BULLETS SHIPPED, batch 277, `ezplate-v226`; TWO SPLIT OUT as items 98 and 99**

✅ **Shipped:** U1 (the Edit-menu-item modal renders cost, suggested and a live food-cost % from `menuMarginPreview`, recoloured as you type) and U22 (money inputs pad to two places).
⚠️ **U22 SHIPPED AS PADDING, NOT ROUNDING, AND THE ITEM'S LITERAL INSTRUCTION WOULD HAVE WRITTEN A WRONG COST.** It said "format to two decimals" and named `#ig_price`. That field holds a price PER KG/L/UNIT (`perDisplayValue` multiplies the stored cost by 1000 for g and ml) and `saveIngEdit` reads it straight into `cost_per_base_unit`. **Measured over the 384 priced rows of `tests/fixtures/base-products.json`: 44 carry more than two decimals, 7 are under a cent, and the smallest is $0.0056/kg** - which `toFixed(2)` turns into $0.01, a 79% increase, on a stored cost. `padMoney` pads and never rounds, and a property test over the whole fixture pins it.
➡️ **U20 (New vs Edit product parity) is now item 98** - a form rebuild that moves which field the stored cost derives from, with `ig_unit`'s deliberate `disabled` to preserve.
➡️ ~~**The precision bullet is now item 99** - there are three different percentages across six-plus sites, not one ratio at two.~~ ✅ **SHIPPED, batch 283 (`ezplate-v230`).**
**And item 99 was itself short by two sites, which is worth recording against the bullet that spawned it.** It named "six consumers" of `menuMarginPreview().pct` and missed `renderEditMargin` — the Edit-menu-item modal THIS VERY ITEM (U1) had shipped in 277 — and it dropped `vbadge`, the Menu row, which is the site the original bullet was actually about. It also cited `js/app.js:12186` for the publish dialog, which is unrelated delete-plate code.
**The substance held and the taxonomy did not:** there were not three quantities at three precisions but ONE quantity computed TWICE (whole number in `menuMarginPreview`, one decimal in `vbadge`), one dead field (`analyze().absPct`, trimmed), and one different quantity that was already consistent (`avgFoodCostForScope`, untouched). Fixed as one computation (`foodCostPct`) and one formatter (`fmtFoodPct`) at one decimal.

⚠️ **FOUR OF THIS ITEM'S POINTERS ARE WRONG AND ONE OF THEM WOULD HAVE CHANGED THE FIX** (batch 277's premise check, 16 Sep 2026). Corrected in place per `CLAUDE.md`'s standing rule; the defects themselves all reproduce.
- `#editModal` is at **`index.html:1587`**, not the 1440 region.
- **`openEditModal` DOES NOT EXIST.** The modal is populated by **`openMenuEdit`** (`js/app.js:15412`) with **`setEditMode`** (`:15498`).
- ⚠️ **`publishPlan` DOES NOT COMPUTE COST, SUGGESTED OR %.** It returns `{action, existingId, unlinked}` — a create-or-update decision. The figures come from **`menuMarginPreview`** (`js/app.js:12155`), which calls **`analyze()`**. *This is the correction that matters: the bullet says "render it, do not recompute it" and then names a function with nothing to render, which is an instruction to go looking and find nothing.*
- **`packToUnitCost` is NO LONGER untouchable.** The parser-region protection was **lifted 10 Sep 2026** (Max: *"its lifted"*), and `.claude/rules/invoice.md` names that function as one of the four now editable. The bullet's caution is stale, though its advice — call it, do not change it — is still the right call here.

- **Edit menu item (U1):** opened from a row showing cost $3.88, suggested $12.94 and 32.4% over, the modal (`#editModal`; `openMenuEdit`) shows name, menu, category and price only. Show cost, suggested price and a live food-cost % under the price field, recoloured by the target rule as the user types. **The figures already exist in `menuMarginPreview`/`analyze`; render them, do not recompute them.**
- **New product vs Edit product (U20):** New asks pack size, pack unit, pack price; Edit asks unit type, price per unit, pack size (optional), so 10 kg for $65 reopens as 6.5 per kg with an empty pack. Make Edit mirror New with the derived unit cost read-only beneath. The form calls `packToUnitCost`; it does not change it.
...
**Every cited line number points at unrelated code** - 4884 is a block comment about `logAllMenuPrices`, 1652-1670 is `bootstrapSync`'s tenant gate, `index.html:1206-1208` is the Account plan card, `:1514-1516` is the Confirm dialog. The placeholders are real and live at `index.html:1269`, `:1620`, `:1625`, `:1626`, against `:1645`'s capitalised one.
**Two sub-items are wrong about the mechanism, not just the address.** The dropdown's forced capitals are on `.opt .ca` / `.sug-opt .ca` (`css/style.css:591`, `:620`) - **the category tag, not the product name**; `.nm` carries no transform, and in `renderDrop` the `.nm` is the kitchen-ingredient name, which is not a product name at all. And the PACK SIZE example **is already split into a `.hint` span** (`index.html:1624`), so the remedy this item asks for is shipped; what still renders it in capitals is `.field label{text-transform:uppercase}` (`css/style.css:502-504`) inheriting into the span, which is a CSS fix and not a markup one.
**What held:** the double space is real and there are THREE of them, not one (`HERBS  SPICES & SEASONINGS`, `SAUCES  CONDIMENTS & DRESSINGS`, `SAUSAGES  HOT DOGS & PATTIES`, in `tests/fixtures/base-products.json`); the category display/storage split is real and `catLabel` already title-cases at the display boundary on purpose, with the storage side load-bearing for the invoice parser's matching; the Tidy-lists mixing is real and `tidyValuesCombined`'s comment records it as **Max's call**, so splitting it reverses a decision of his and needs him.
- **Product identity (U17):** Ingredients shows Product, Brand, Supplier; the builder dropdown shows Product only in forced capitals; the docket shows Product, Brand; Products shows Product with Brand as a tag and supplier only through the filter. One line everywhere: Product, then Brand muted, Supplier where known. ~~Sites: `renderKingRows` (`js/app.js:4884` region), the `#drop` option renderer (`:1652-1670`), the docket line (`:1767` region), `renderIngRows` (`:4638` region).~~ **Struck: see the premise check above.**
...
- **"✎ Manage list…" inside the category `<select>` (U8; `TIDY_DOOR`, built in `fillFilter` — the old `js/app.js:4549` citation was already stale, it is `:5568` at v216 and will move again):** a small link beside the filter instead of an option that is a command.
  ⚠️ **PARTLY MOVED, batch 267 (`ezplate-v216`), and this bullet is NOT satisfied.** Item 57 asked for the valued and valueless options to be split, so `fillFilter` now emits the real values in one `<optgroup>` and the door in its own, headed "Edit this list" — the command is no longer a silent sibling of a value, and it is no longer the last flat entry a hurried thumb lands on. **That is a mitigation, not this bullet's remedy:** it is still an option, still inside the filter, and still reached by "choosing a filter". Decide here whether the optgroup is enough or the door comes out of the select altogether; if it comes out, delete the `sel.dataset.tidyField` branch and the `TIDY_DOOR` handler at `:10663` together, and check the phone first — a native select sheet is where the door is most confusing and a link beside the filter costs a row of vertical space the phone does not have.
- **`.mnu-sec` prints a section name in forced capitals (`MAINS`) while `.plib-cat`, `.ad-meta` and this screen's own category filter all say `Mains`** — considered and deliberately kept by batch 267 as a group heading rather than a shout, and recorded in `docs/MAINTENANCE.md` with the argument. It belongs here rather than to a casing sweep because changing it changes this screen's rhythm and wants deciding beside the chip-vocabulary bullet above, in one look at one screen.
...
- ~~`edDelArmed` (declared, written twice, read nowhere); `analyze().absPct` (no reader since v122); `avgFoodCostForScope` counts dishes whose `menuId` has no By-menu row (latent, zero today; item 18 opens the function); `verdictHtml`'s "Nothing costed and priced on this menu yet" branch unreachable for a named menu; `ingredients.updated_at` is not history (make it honest or drop it); `ing_price_history`'s `unique (product_id, recorded_at)` needs its own brief; the ~390KB of self-hosted fonts re-download on every deploy (a versionless font cache) and `cache.addAll`'s swallowed partial install. From `docs/MAINTENANCE.md`; each rides the batch that opens its function.~~ ✅ **DONE, batch 272 — struck by AUDIT-v227's pre-push review.** `edDelArmed` has zero hits in `js/app.js`; it went with the edit-modal footer pass. ⚠️ **It was struck in `docs/MAINTENANCE.md` in the same batch and left standing HERE**, which is one dead fact in two files — the shape `docs/QUEUE-GROUPS.md`'s header warns about. Caught by the reviewer, not by the audit that found the other half.
