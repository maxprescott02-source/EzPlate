...
❌ **"MOVE IT INTO THE DOCKET HEADER" WAS NOT DONE, DELIBERATELY, AND THIS IS THE RECORD SO IT IS NOT RE-PROPOSED.** Three measurements, all at 380:
  ...
  - below 768 `.bld-actrow` is **the only thing `#bCost` still shows** (`.bld-sumhead`, `.bld-kv`, `> #saveBtn` and `> #saveHint` are all `display:none`, and `.is-bare` hides the body), so moving Clear out leaves a bordered card containing one button;
