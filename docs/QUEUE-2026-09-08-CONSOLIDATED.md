...
## next  90 · Item 21's remaining halves: a delete sequence and a boot-time replace  **[B, unmeasured; the completion message SHIPPED in batch 253]**
...
**WHAT IS LEFT, and the first of these is a UX question rather than a defect:**
...
- **`doDeleteMenu`'s unawaited dish deletes**: the change-log entry chains off the write that decides the menu is gone, and the dish deletes are fired without being awaited. No FK to violate (`menu_items.menu_id → menus.id` is ON DELETE SET NULL), so this is the v112 sequencing class rather than a 23503 risk. The comment at the site says it is out of scope for the batch that wrote it.
- **`priceHistory` wholesale replace at boot**: an empty or filtered server response replaces local, while `menuHistory` merges — and the comment at that very site explains why merging is right and says the all-menus series was left "untouched here deliberately". A point logged offline is lost at the next sync. Make it merge like its siblings or say at the site why not.
