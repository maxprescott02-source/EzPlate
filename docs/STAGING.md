...
### And on 29 Aug 2026 (batch 218), `create_business` — the rehearsal that found a grant that was never there

The migration was written in batch 209 and could not be rehearsed, because this project was paused. It sat on an open PR for two days and was applied here first.

- **Step 2 by measurement for the third time running, and it is now simply the way to do it.** All seven fingerprints matched production before anything ran, so `01-schema.sql` was not re-run. After the migration they matched again with `functions_fp` moved 9 → 10 on both sides.
- **Step 3 was deliberately SKIPPED, and saying why is the point.** The case this migration is about is an AUTH state — a confirmed account belonging to no café — not a data shape, and **a seed does not touch `auth.users`**. `c@example.com` already was that state. Loading `03` or `04` would have churned 500 rows and proved nothing about the function. **Read step 3 as "make sure the case exists", not as "always run a seed".**
- ⚠️ **THE DEFECT, and it is the reason this file says to verify as the client rather than through the MCP.** The migration ended `revoke all on function … from public;` then `grant execute … to authenticated, service_role;` — the idiom every migration here uses — and its comment claimed *"two mechanisms, because a grant is checked before the body runs."* Measured as the anon client: `P0001 sign in before creating a cafe`, HTTP 400. **That is the BODY refusing, not the grant.** Supabase's default privileges grant `anon` EXECUTE at CREATE time and `revoke … from public` does not touch it. After adding `revoke execute … from anon`: `42501`, HTTP 401. Both are refusals, which is exactly why it survived review, a green suite and a test written to forbid it. `CLAUDE.md` now carries the mechanism.
...
### And on 10 Sep 2026 (batch 250), the two history series — the rehearsal that could sign in AS THE ROLE

`20260910_history_delete.sql`. Queue item 89's server half: an owner-only DELETE on `price_history` and `menu_price_history`. Three things belong here because they are about the PROCEDURE rather than about that migration.

- **Step 2 by measurement for the fourth time running.** All seven fingerprints matched production before anything ran, so `01-schema.sql` was not re-run. Afterwards they matched again with `policies_fp` moved 25 → 28 on both sides. **Diffing first is strictly better than re-running and assuming**, and this is now simply the way to do it.
- ⚠️ **THE STAFF REFUSAL WAS ACTUALLY EXERCISED, WHICH IS THE THING 218 AND 219 BOTH COULD NOT DO.** `CLAUDE.md` records the shape: *"when a function's guard names a role, the rehearsal has to sign in AS that role"* — and 219 shipped a hole precisely because staging had only tried `anon` and the OWNER. **Staging's seed carries a staff membership** (user `4444…`, business `0000…0001`), so this one could. In one transaction, `set local role authenticated` plus that user's JWT claims: staff deleted **0 of 1** on each series, the owner then deleted **1 of 1** on each, and the test rows were removed.
  **You do not need the missing test ACCOUNT for this.** Queue item 27 is about a signed-in session over PostgREST, and that is still blocked — but a POLICY can be exercised in-session with `set local role` and `set_config('request.jwt.claims', …)`, which is a genuine RLS evaluation rather than a bypass. **Reach for that before concluding a role cannot be rehearsed.**
- ⚠️ **AND THE HARNESS WAS PROVED TO FAIL, which is what makes the pass evidence.** The same block was re-run with its assertion inverted and raised `staff_deleted=0`. A `do $$ … raise exception … $$` block that completes silently looks identical whether it checked anything or not, and the MCP does not surface `raise notice` — so **invert it once and watch it raise**, or you have a green nobody has watched go red. Same discipline as 219's counterfactual, one construct along.
- **The anon-over-PostgREST check is recorded as proving almost nothing**, deliberately. It returned HTTP 200 with an empty body on both tables — but anon has no tenant, so those rows were already invisible and the result would have been identical before the migration. **An empty representation is the failure signal for a write anon SHOULD be able to make; it says nothing about a restriction on rows anon could never see.**
- **What was left behind:** nothing. Four disposable rows inserted and removed inside the same transaction, leftover count verified 0. No account was consumed, no password reset.
