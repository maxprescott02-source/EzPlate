...
### `screenshots.spec.js` has been RED since 186, and nothing anywhere reports it
Measured 14 Aug 2026 during batch 188, on `origin/main` in the maintenance worktree so the batch could not be blamed: **1 passed, 13 failed.** Every screen walk fails.
**The cause is 186 doing exactly what it was built to do.** This is the one spec that does NOT call `installBoot` — it drives the real app against the café's live production database — and 186 made sign-in mandatory, so it now lands on the sign-in gate and finds no screen to photograph. There is no bug in the app; the spec's premise expired.
**Why nobody noticed, and why that is the real item.** It is deliberately excluded from the CI job (it is non-hermetic, and `tests/ci-workflow.test.js` pins the exclusion), and it needs credentials the harness will never have. So the only signal is a local full run, where it reads as 13 familiar-looking failures at the bottom of a green-looking suite. **A spec that cannot pass and cannot report is worse than a deleted one**: it trains every future batch to skim past a block of red, which is the state a real regression would arrive in.
Requirements: decide between the two honest answers and take one. **Delete it**, since `installBoot`-based specs now cover every screen at both widths and both themes; or **keep it and make it skip explicitly** (`test.skip` with the reason, or a guard on a credential env var) so a full run says "skipped, needs a signed-in session" instead of "failed". Either way `tests/ci-workflow.test.js`'s spec-count assertion moves with it.
