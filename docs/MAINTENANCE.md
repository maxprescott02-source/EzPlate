...
Filed 23 Aug 2026 by batch 201, which added an explanation to the invoice review and measured whether a user could read it.
...
`tests/visual/201-pack-unit.spec.js` MEASURES it every run and asserts a floor of **3.0** — the AA floor for large text and UI components — with the shortfall written out at the assertion. That is deliberate: asserting 4.5 leaves a permanently red test that says nothing new, and asserting 4.17 pins the defect as though it were intended. Raise the number in that spec as part of the fix.
...
### ~~`tests/visual/screenshots.spec.js` cannot pass and cannot report it~~ — **EXECUTED, batch 201 (`ezplate-v170`)**
...
### ~~`var catState` is declared TWICE at top level, and the duplicate guard cannot see it~~ — **DONE, batch 201 (`ezplate-v170`)**
