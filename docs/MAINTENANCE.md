...
### `tests/visual/screenshots.spec.js` cannot pass and cannot report it — DECIDED, execute the decision
...
### Three code comments and a missing one, all routed by AUDIT-v166 — **do these together, in the next batch that already bumps the cache**
Batch 194 was prose-only, so it fixed the `CLAUDE.md` half of each of these and could not fix the half that lives in a shipping file without inventing a version bump for a handful of comments. **All four are in `js/app.js`; none changes behaviour.** Grouped into one entry deliberately: separately they are four ten-minute jobs that will each keep losing to real work, and together they are one.

1. **`setProduct` is described as `ing_price_history`'s sole writer, and since 193 it is not.** `grep -n "sole writer" js/app.js` — the comment says *"every product-price write in the app funnels through `setProduct` (v109), which is ing_price_history's sole writer."* The real writer is **`setProducts`** (plural); `setProduct` is its N=1 wrapper, and the catalogue importer calls the plural directly with up to a whole catalogue. The behaviour is correct — this is only the comment. `CLAUDE.md`'s two copies of the claim were corrected in 194.
2. **`buildBackup` is missing the non-bump note that `HANDOVER-193` says is already there.** 193 added `supplier_code` to every in-memory product and deliberately did not bump `stamp.format`, which was the right call — but the note recording the decision was never written, so the handover cites a comment that does not exist. 184 hit the identical tension and wrote its note one line from where this one goes, with the reason: *a silent decision against this rule is indistinguishable from having missed it.* Say: no group added, removed or renamed; no key changed type; nullable with no default so both directions read null; format 4 is reserved by the backup-history queue item. `CLAUDE.md`'s rule was given its carve-out in 194 and now requires this note explicitly.
3. **Two `:not([hidden])` comments in `css/style.css` state the override mechanism backwards** — see the entry below, which has the detail and has been open since 10 Aug.
4. **Two `css/style.css` comments still say "twelve" `:not([hidden])` rules** (`grep -n "twelve" css/style.css`). The live figure was 13 at v166 and this number has now been wrong in four places at once. **Do not re-count it** — `CLAUDE.md`'s enumeration was deleted in 194 rather than corrected a third time, and these two should point at the grep the same way.
...
**Find them with `grep -n outranks css/style.css`** — one above `.plib-controls`, one above `.plib-note`. The first says a bare `display:flex` "outranks the UA's `[hidden]{display:none}`"; the second says the same of `display:block`, plus "(class beats attribute-less type rules)".
⚠️ **This entry cited `:3263` and `:3378-3380` until 15 Aug 2026 and both had drifted ~440 lines** — they are near `:3700` and `:3844` as at v166, and will move again. Re-pointed by NAME per this file's own header, which says exactly that and which this entry was ignoring. (AUDIT-v166 T3, which noted the irony: a stale line-number citation inside the entry about a stale explanation.)
...
⚠️ **OPEN SINCE 10 AUG 2026 — 17 batches, 10 deploy versions — and the reason it was parked has EXPIRED.** `HANDOVER-177` left it *"as a flag rather than a proposal, because only Max…"*; the **13 Aug standing-authority reversal made every documentation and comment call the assistant's own**, so nothing is waiting on him and nothing has been since. AUDIT-v166 (D2/R4) flagged that it crossed its own "if it happens a third time it is a rule" threshold and still was not touched.
**It lands in a client asset, so it takes a cache bump and the mandatory review — which is the whole reason it has never ridden a batch of its own.** Do not give it one: **attach it to the next batch that is already bumping the cache.** Two reworded comments add nothing to that batch's review surface, and a prose-only batch cannot carry it without inventing a version bump for two comments.
