...
- **A failed `menus` SELECT is treated as a valid boot and mints a menu that is not on the server.** ⚠️ UNMEASURED; **promote to B on reproduction, and it would deserve it.** Claim: `menus` is excluded from the fatal bootstrap-read check (`js/app.js:1139`), so a transient failure of that one request lets boot proceed; `ensureDefaultMenu()` then generates an in-memory "Original menu" id that was never inserted. Existing `menu_items` still carry the real server ids and stop matching, so it **first presents as "my dishes disappeared"**; the publish guard around `2712-2719` then sees a non-empty `menusList`, assumes a server menu exists, skips the create path, and a write with the fictional id hits the FK. Claimed sites `js/app.js:1139, 2626-2646, 2712-2719` — grep, do not trust. **The reviewer's sharpest test observation is attached to this one:** `menu-default.test.js` is an *expensive green test* — it asserts the invariant that `menusList` holds server menus at the explicit create-menu entry point, while bootstrap violates the same invariant on a path the test never exercises. `boot-gate.test.js` does not cover `menus` as a bootstrap dependency either. **Note the ordering rule this brushes against and does NOT contradict:** the `ensureDefaultMenu` gate belongs at its call site, and a successful EMPTY read is the user having deleted everything and must be respected. This is the third case — a read that FAILED — which the two-valued gate cannot express. Same family as 185.
