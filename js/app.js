...
/* Item 7 — About/Privacy/Contact modal wiring */
(function(){
  var v=document.getElementById('infoVersion'); if(v) v.textContent=APP_VERSION;
  var fv=document.querySelector('.side-foot-ver'); if(fv) fv.textContent=APP_VERSION;
  function openInfo(){ show('infoModal'); }
  function closeInfo(){ hide('infoModal'); }
  ['infoOpenMobile','infoOpenDesktop'].forEach(function(id){ var b=document.getElementById(id); if(b) b.addEventListener('click', openInfo); });
  var c=document.getElementById('infoModalClose'); if(c) c.addEventListener('click', closeInfo);
  var m=document.getElementById('infoModal'); if(m) m.addEventListener('click', function(e){ if(e.target===m) closeInfo(); });
})();
