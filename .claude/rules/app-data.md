...
**`menusList` MEANS MENUS THE SERVER HAS, and `withPublishMenu` decides whether to create one by reading `menusList.length`** - so a writer that puts a menu there before the server has kept it puts a hole straight through the publish path, and the app then writes a dish against an id no `menus` row answers to. **FOUR writers, all of which wait for the server:** `bootstrapSync` assigns what the table returned, `submitNewMenu` pushes and takes back anything the server refused, `ensurePublishMenu` pushes only after a confirmed write, and `rollbackMenuDelete` re-inserts a menu at its original index when the server REFUSED the delete. **If you add a fifth, this is what it owes.**
⚠️ **This said THREE until AUDIT-v227, and the fourth landed in batch 254.** The invariant was never violated - a rollback restores a menu the server still holds, which is the opposite of inventing one - but the count is the exact "exactly N" form this file tells you to grep rather than believe, and the next person to add a writer would have counted three and stopped.
