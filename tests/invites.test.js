...
test('each restriction names its ONE command, and none of them says `for all`', () => {
  ...
  for (const cmd of RESTRICTED) {
    const stmt = policyStmt(MIG, `business_invites owner-only ${cmd}`);
    assert.match(stmt, new RegExp(`for ${cmd}\\b`), `the ${cmd} restriction must restrict ${cmd}`);
    assert.ok(!/for all\b/.test(stmt),
      `the ${cmd} restriction covers every command — staff could not even READ, which is not the design`);
  }
});
...
test('the restrictions are keyed to OWNER, and NULL — no membership — refuses', () => {
  ...
  for (const cmd of RESTRICTED) {
    ...
    assert.match(stmt, /current_business_role\(\)\) = 'owner'/,
      `the ${cmd} restriction must require owner outright, not merely "not staff"`);
    assert.ok(!/<>\s*'staff'/.test(stmt),
      `the ${cmd} restriction reads "not staff", which a caller with NO role also satisfies`);
  }
});
...
test('RLS is enabled BEFORE every policy on the new table, not merely somewhere in the file', () => {
  ...
  const grant = MIG.indexOf('grant all on table public.business_invites');
...
test('the mirror is re-runnable — every new object is idempotent', () => {
  ...
  assert.match(MIR, /'app_settings','business_invites'/,
    'business_invites must be in the grants loop, or PostgREST cannot see the table at all');
});
