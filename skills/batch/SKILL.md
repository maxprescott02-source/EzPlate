...
**Since batch 284 that file is a capped WORKING LIST, and `npm test` enforces it** (`tools/maintenance-check.js`): an entry you finish is **DELETED, not struck**; a new entry opens with an `Anchor:` line or the suite goes red; and past the cap, adding one means deleting one. **Grep it for every function you change before you push** - the anchor catches a subject that is deleted, not a defect fixed in place.
**If the queue ever has nothing unblocked left, REFILL IT FROM THE NEXT GROUP FIRST; only if there is nothing left to promote does a maintenance sweep run as its own ordinary batch.**
⚠️ **This said "a maintenance sweep runs" with no refill step until 9 Sep 2026, and from the 8 Sep consolidation onwards that was wrong in the quiet direction.** `docs/QUEUE.md` is capped at 20 and is the WORKING SET; `docs/QUEUE-2026-09-08-CONSOLIDATED.md` is the backlog (a grep for unstruck items, never a count — it was 72 on 8 Sep and 73 by 9 Sep), promoted a group at a time in the order `docs/QUEUE-GROUPS.md` sets. A drained queue therefore means *"this group is finished"*, **not** *"the work is finished"* - and a loop that reads it as the second would start a maintenance sweep with sixty items outstanding, having been told by the file that everything was done. **Nothing in the queue can say which of the two it is**, which is exactly why the refill is a step here rather than a note in a doc.
**The refill, and every part of it is derived so there is no status field to go stale:** the current group is **the first group in `docs/QUEUE-GROUPS.md`'s order with unstruck items**; the items to promote are **that group's unstruck items** (step 10 strikes each one as it ships); promote up to the cap, in the group's stated internal order, and say in the handover which group opened. **If the top group's items are all blocked, take the next group's** - a blocked group does not stall the ones behind it.
...
1. **Take the top unblocked item.** Mark it in progress in `docs/QUEUE.md`.
   ...
   **Then run the `premise-check` agent on the item, BEFORE any edit and before planning.** Read-only, Read/Grep/Glob, Sonnet: it greps every symbol, path, line number and count the item names and returns claimed-against-found in under 400 words. `.claude/agents/premise-check.md` is the definition.
   **Record its claim-vs-found in the handover**, in one line, whichever way it came out - "every claim held" is the result that makes the other ones legible.
   ⚠️ **It reports; it does not decide.** If it finds the enumeration short, the item is REWRITTEN in `docs/QUEUE.md` to match reality and the batch carries on - that is `CLAUDE.md`'s standing rule, not a stop condition and not a re-ask.
   **Why an agent rather than a line telling you to grep:** the line already exists, in `docs/QUEUE.md`'s own header, and **seven of the last eleven batches found their item's enumeration short anyway.** Nobody skips the count on purpose; they skip it because the item sounds definite and the grep is boring. A separate reader with no stake in the plan is the cheapest thing that breaks that.
   **And read the rule file for what you are about to touch, if you are planning against a file you have not opened.** Since batch 264 the traps live in `.claude/rules/*.md`, scoped with `paths:` frontmatter, so **Claude Code loads them when it reads a matching file** - `js/app.js` pulls in `app-guards.md`, `app-data.md` and `invoice.md`, `supabase/**` pulls in `sql.md`, and so on. That covers editing. It does **not** cover planning, estimating or arguing about a file you have not read, which is where a wrong premise is cheapest to form and most expensive to act on. `docs/rules/process.md` holds the full record behind every process rule in `CLAUDE.md` and is loaded by nothing - read it by hand when a procedure matters.
...
10. **Hand over.** Use the `handover` skill, and draft it with the `handover-writer` agent - Haiku, given the branch diff and the template, returns a body under 60 lines. You review and fix it; the file is yours, the draft is its. `.claude/agents/handover-writer.md` is the definition.
    **THEN RUN `node tools/state.js`, IN THE SAME COMMIT AS THE HANDOVER.** It rewrites `docs/STATE.json` - batch, deploy version, current group, open item count, newest audit, newest handover - deriving every field from the files so there is no status marker to go stale. `tests/state-file.test.js` asserts its batch against the newest handover, so **a handover landing without this run turns `npm test` red**, which is the mechanism rather than an accident.
    **DELETE the finished item from `docs/QUEUE.md`** - git and the handover hold the record, and a done section is how that file reached 979 lines.
    ⚠️ **AND IF THE ITEM CAME FROM `docs/QUEUE-2026-09-08-CONSOLIDATED.md`, STRIKE IT IN **BOTH** BACKLOG FILES IN THE SAME EDIT** - there and in `docs/QUEUE-GROUPS.md`'s `**Items:**` line for its group - with the batch number and the deploy version it shipped (or "no client asset"). **Deleting from the queue and not striking the source re-offers finished work to the next promotion**, which is the same defect one level up from the `Do after:` line that sent an item waiting two years on a conversion that had already landed.
    ⚠️ **THE GROUPS FILE WAS NOT NAMED HERE UNTIL 15 SEP 2026** (batch 270, found by its pre-push review), and three shipped items had accumulated there unstruck while the consolidated file recorded all three as done. **The derivation was never wrong** - `tools/state.js` reads struck-ness from the consolidated file, so the machine had the right answer - **which is exactly why it went unnoticed for three batches: the only reader it misled was a human deciding whether a group was finished.** Two files describing one item is two files that will disagree, and that file's own header says so about its prose while its `**Items:**` line is a field.
    ...
    **A finding defaults to C and lands in `docs/MAINTENANCE.md`**, as an anchored entry (that file's header has the format). It enters `docs/QUEUE.md` only if it would stop, embarrass or hurt a paying customer at launch - and if that file is already at its 20-item cap, it displaces the lowest-priority item there into maintenance rather than extending it.
