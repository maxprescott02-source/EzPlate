...
⚠️ **This said "a maintenance sweep runs" with no refill step until 9 Sep 2026, and from the 8 Sep consolidation onwards that was wrong in the quiet direction.** `docs/QUEUE.md` is capped at 20 and is the WORKING SET; `docs/QUEUE-2026-09-08-CONSOLIDATED.md` is the backlog of 72, promoted a group at a time in the order `docs/QUEUE-GROUPS.md` sets. A drained queue therefore means *"this group is finished"*, **not** *"the work is finished"* - and a loop that reads it as the second would start a maintenance sweep with sixty items outstanding, having been told by the file that everything was done. **Nothing in the queue can say which of the two it is**, which is exactly why the refill is a step here rather than a note in a doc.
